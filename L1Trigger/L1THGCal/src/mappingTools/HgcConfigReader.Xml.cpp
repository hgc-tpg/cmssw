...
namespace l1thgcmapping {
  ...
  template< typename T , typename U , typename V , typename W  >
  void ReciprocalInsert( const DOMDocument* doc , const std::string& aXpath , const T& aChildIdFn , const U& aParentIdFn , V& a_to_b , W& b_to_a )
  {
  ...
  const auto DecValue = []( const char* value ){
    return std::stoul(value, nullptr , 10 );
  };
  const auto HexValue = []( const char* value ){
    return std::stoul(value, nullptr , 16 );
  };

  const auto DecId = []( const DOMElement* element ){
    return std::stoul(getNode(element, "id"), nullptr , 10 );
  };
  const auto HexId = []( const DOMElement* element ){
    return std::stoul(getNode(element, "id"), nullptr , 16 );
  };
  const auto HexHref = []( const DOMElement* element ){
